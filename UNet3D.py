...
The 5 chnnels  are  five MRI contrasts (FLAIR, PD-weighted, T2-weighted, T1 pre-contrast, and T1 post-contrast).
...
def get_data_single3d(input_file_name):
    
    image_id=input_file_name[0]
    #print(image_id)
...
def predict_single_image(g, real_imgs):
...
g.compile(optimizer=Adam(lr=initial_learning_rate), loss=dice_coef_loss,  metrics=['accuracy'])
g.summary()
...
for n_round in range(nb_epoch+1):
        ...
        for current_batch_index in range(0,len(train_id),batch_size):
            ...
            for k in range(0,256-img_cols+1,int(img_cols/2)):           #select 3D patch from the scan
                    for j in range(0,256-img_cols+1,int(img_cols/2)):
                        for i in range(0,65-img_hight,int(img_hight/2)):
                            imgs=current_batch[:,k:k+img_rows,j:j+img_cols,i:i+img_hight,:]
                            imgs_mask=current_label[:,k:k+img_rows,j:j+img_cols,i:i+img_hight,:]
                            
                            if (np.sum(imgs_mask) > 2):
        ...
        print_metrics(n_round+1, train_loss_sum=np.sum(train_loss), train_loss_mean=np.mean(train_loss))
        ...
        if n_round%1==0: #goes for validation
            ...
            for i in range(0,len(val_id),batch_size):
                batch_id=val_id[i:min(i+batch_size,len(val_id))]
                real_imgs,real_mask=get_data_single3d(batch_id)
                pred_mask=predict_single_image(g, real_imgs)
                pred_mask[pred_mask>.9]=1
            ...
            print('n_round',n_round,'Val_dicefp_mean',Val_dicefp_mean)
            if n_round%50==0 :
